...
import { Card, Radio, Button, Typography, Space, Progress, Layout, message } from 'antd';
...
const ImprovedPCExamPage: React.FC = () => {
    ...
    const [submitted, setSubmitted] = useState(false); // 新增标记是否提交了全部答案
    const [isSubmitting, setIsSubmitting] = useState(false); // 新增状态变量
    ...
    const handleSubmitAll = async () => {
        ...
        try {
            ...
            if (response.code === 0) {
                ...
                setCorrectAnswers(newCorrectAnswers); // 保存正确答案
            } else {
                ...
                <Sider width={300} style={{ background: '#fff', padding: '24px' }}>
                    <Space direction="vertical" size="large" style={{ width: '100%' }}>
                        ...
                        <Progress
                            ...
                            status="active"
                            type="circle"
                            format={(percent) => `${formatTime(timeLeft)}`}
                        />
                                ...
                                <div key={question.id} style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <div>
                                        <Paragraph>
                                            ...
                                            {`${((currentPage - 1) * 5) + (index + 1)}、${question.subjectsTitle}`}
                                        ...
                                        <Radio.Group
                                            ...
                                            disabled={submitted} // 如果已经提交，则禁用选择
                                        >
                                                ...
                                                {selectedAnswers[question.id] === correctAnswers[question.id] ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <Paragraph type="danger">
                                                            你的答案: {selectedAnswers[question.id]} <br />
                                        ...
                                        {submitted && (
                                            <div style={{ marginLeft: '0px' }}>
                                                {selectedAnswers[question.id] === correctAnswers[question.id] ? (
                                                    ...
                                                    <Paragraph type="danger">
                                                        你的答案: {selectedAnswers[question.id] ? selectedAnswers[question.id] : '没有作答'} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                        正确答案: {correctAnswers[question.id]}
                                                    </Paragraph>
                                                )}
                                            </div>
...
export default ImprovedPCExamPage;
